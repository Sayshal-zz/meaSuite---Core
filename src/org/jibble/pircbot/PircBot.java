 ...
 * <p>
 * Please visit the PircBot homepage at <a href="http://www.jibble.org/pircbot.php">http://www.jibble.org/pircbot.php</a> for full revision history, a beginners guide to creating your first PircBot and a list of some existing Java IRC bots and clients that use the PircBot framework.
 * 
...
public abstract class PircBot implements ReplyConstants {
	...
	/**
	 * Sends a CTCP command to a channel or user. (Client to client protocol). Examples of such commands are "PING <number>", "FINGER", "VERSION", etc. For example, if you wish to request the version of a user called "Dave", then you would call <code>sendCTCPCommand("Dave", "VERSION");</code>. The type of response to such commands is largely dependant on the target client software.
	 * 
	 * @since PircBot 0.9.5
	 * 
	 * @param target
	 ...
	 * 
	 * @return a DccChat object that can be used to send and recieve lines of text. Returns <b>null</b> if the connection could not be made.
	 * 
	 * @see DccChat
	 */
	...
	/**
	 * This method is called by the PircBot when a numeric response is received from the IRC server. We use this method to allow PircBot to process various responses from the server before then passing them on to the onServerResponse method.
	 * <p>
	...
	/**
	 * Called when the mode of a channel is set. We process this in order to call the appropriate onOp, onDeop, etc method before finally calling the override-able onMode method.
	 * <p>
	 ...
	 * @since PircBot 0.9.9
	 * 
	 * @return The last password that we used when connecting to an IRC server. Returns null if we have not previously connected using a password.
	 */
	 ...
	 * <ul>
	 * <li>This method may not return a full list of users if you call it before the complete nick list has arrived from the IRC server.</li>
	 * <li>If you wish to find out which users are in a channel as soon as you join it, then you should override the onUserList method instead of calling this method, as the onUserList method is only called as soon as the full user list has been received.</li>
